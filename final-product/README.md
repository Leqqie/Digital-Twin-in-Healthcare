...
The data is expected to come from the OhioT1DM dataset, split into 2 different folders within the data folder: one for training and one for testing. for training it assumes {train_dir}/{patient_id}-ws-training.xml and for testing it assumes {test_dir}/{patient_id}-ws-testing.xml
